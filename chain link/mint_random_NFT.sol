...
contract minting is ERC721, VRFConsumerBase{
    using SafeMath for uint256;
    using Strings for uint256;

    uint256[1500] public collection1;
    ...
    function fulfillRandomness(bytes32 /*requestId*/, uint256 randomness) internal override {
        ...
        randomResult = randomness.mod(1500).add(1) ;
    }
   
    function Mint() public  {
       uint256 randomId2 =  uint256(getRandomNumber()) % collection1.length;
        _safeMint(msg.sender, randomId2);
        checkId[msg.sender] = randomId2;
        tokenId1.push(randomId2);
        collection1[0]--;  
    }
