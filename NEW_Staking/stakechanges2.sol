...
contract GemStaking{
    ...
    uint256 maxCycleTime = 2 minutes;
    uint256 public cycleSlots = 2;
    ...
    function cycleReward (address _user,uint256  _time) public view returns(uint256){
        ...
        for(uint256 i; i< cycleDiff; i++){
            ...
            cycleTime = ((user.startTime).add(_time.mul(60))).sub(cycles[(user.cycleStake) + 1].cycleStartTime);
    ...
    function stakeTime(address _user, uint256 userCycle) public view returns(uint256){
        ...
        if(block.timestamp <= cycles[userCycle].cycleEndTime){
            timeDiff = ((block.timestamp).sub(user.startTime)).div(60);
        }
        else {
            timeDiff = ((cycles[userCycle].cycleEndTime).sub(user.startTime)).div(60);
        }
        return timeDiff;
    }
    ...
    function userReward(address _user) public view returns(uint256){
        ...
        uint256 nextReward;
        if(userCycle< curCycle){
            nextReward = cycleReward(_user,time);
        }
