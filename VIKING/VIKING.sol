...
abstract contract Context {
 ...
 library SafeMath {
  ...
  contract THEVIKINGS is ERC721A, Ownable{
    ...
    function InitMinting(address[] memory _recipients) internal {
        for (uint256 i = 0; i < _recipients.length; i++) {
            ...
            addMintedaddress(_recipients[i]);
        }
    }
    ...
    {
        require(Royal_Sale, "ROYAL_NOT_STARTED");
        ...
        addMintedaddress(msg.sender);
    }

    //  2nd sale
    function JarlsVikingsSale(uint256 _count) public
    {
        require(Jarls_Sale, "JARLS_NOT_STARTED");
    ...
    {
        require(Karls_Sale, "KARLS_NOT_STARTED");
    ...
    function addMintedaddress(address _to) internal {
      bool _isAvailable = isAlreadyAvailable(_to);
        if(!_isAvailable){
            NFTHolders.push(_to);
        }
    }
